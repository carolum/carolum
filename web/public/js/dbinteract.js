...
async function setNotesFromJournal(mod, ret, requestSetPtr, amtExpected) {
    ...
    for ([key, val] of Object.entries(ret.ids).reverse()) {
        noteObject = await firebase
            .database()
            .ref('/users/' + firebase.auth().currentUser.uid + '/notes/' + key)
            .once('value')
...
async function setJournalSelectors(mod) {
    ...
    var defaultJournalObject = await ref
        .child('data/defaultJournal')
        .once('value')
    ...
    if (defaultJournalKey != null && defaultJournalKey != '') {
        ...
        var defaultJournal = await ref
            .child('journals/' + defaultJournalKey)
            .once('value')
...
async function deleteJournal(journalID) {
    ...
    for (var noteID in noteChildrenIDs) {
        var childRef = firebase
            .database()
            .ref(
                '/users/' + firebase.auth().currentUser.uid + '/notes/' + noteID
            )
            .child('journal')
...
async function loadJournalToEdit(journalID, ptr, amt) {
    ...
    var ref =
        ptr == '' ? parentRef.limitToLast(amt) : parentRef.limitToLast(amt + 1)
...
async function newJournal(name, isDefault) {
    ...
    var pushedObject = await firebase
        .database()
        .ref('/users/' + firebase.auth().currentUser.uid + '/journals')
        .push({
            name: name,
...
async function getSalt() {
    var salt = await firebase
        .database()
        .ref('/users/' + firebase.auth().currentUser.uid + '/userdata/salt')
        .once('value')
