...
var nav = new Vue({
    ...
    data: {
        tabs:
            ['new', 'dash', 'settings'].indexOf(location.hash.slice(1)) === -1
                ? 'new'
...
var newEntry = new Vue({
    ...
    methods: {
        ...
        update: async function () {
            ...
            var nameAndID = await newNoteWithData(
                this.getData().title,
                dashboard.defaultJournalId,
                this.getData().text
            )
    ...
    methods: {
        createJournal: async function () {
            await newJournal(
                ...
                this.newJournalData.newJournalDefault
            )
        ...
        createNote: async function () {
            var nameAndID = await newNote(
                this.newNoteData.newNoteName,
                this.newNoteData.journalID
            )
